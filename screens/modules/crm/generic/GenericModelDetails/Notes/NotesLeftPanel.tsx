    ...
    try {
      await saveNote({
        ...
        if (
          ...
          setNotesCount(filteredNotesList?.length || 0);
      ...
      <form className="mt-4 lg:mb-2" onSubmit={(e) => e.preventDefault()}>
        {
          <Formik
            enableReinitialize
            initialValues={{
              note: selectedNote?.note || null,
              attachment: selectedNote?.attachment || null,
            }}
            // validationSchema={validationSchema}
            onSubmit={(values, { resetForm }) => {
              if (values.note || values.attachment) {
                handleCreateNote(values).then((response) => {
                  if (response)
                    resetForm({
                      values: { note: null, attachment: null },
                    });
                });
              }
            }}
