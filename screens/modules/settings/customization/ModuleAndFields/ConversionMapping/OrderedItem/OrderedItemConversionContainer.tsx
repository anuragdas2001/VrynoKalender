...
import { OrderedItemConversionMappingList } from "./OrderedItemConversionMappingList";

export const OrderedItemConversionContainer = ({
  ...
  errorMessage: string;
  orderedItemMappingSaving: boolean;
  ...
  return (
    ...
    >
      {({ handleSubmit, setFieldValue }) => (
        <>
          <GenericBackHeader
            heading={`${labelData.orderedItem} Conversion Mapping`}
            ...
            <div className="grid grid-cols-6 sm:grid-cols-4 gap-x-4 mt-5 sm:mt-0 w-full sm:w-1/2 sm:max-w-xs">
              <div className="col-span-3 sm:col-span-2">
                <Button
                  ...
                  buttonType="thin"
                  kind="icon"
                  disabled={orderedItemMappingSaving}
                  userEventName="orderedItemConversion-mapping-clear-click"
                >
                  {t("common:Clear")}
                </Button>
              </div>
              <div className="col-span-3 sm:col-span-2">
                <Button
                  ...
                  kind="primary"
                  disabled={orderedItemMappingSaving}
                  userEventName="orderedItemConversion-mapping-save:submit-click"
                >
                  {orderedItemMappingSaving ? (
                    <Loading color="White" />
                  ) : (
            ...
            <div className="px-6">
              <div className="mb-4">
                ...
                <p className="text-xsm text-vryno-card-value">
                  Map the fields in the {labelData.orderedItem} module to the
                  fields in {labelData.invoicedItem} module. With the mapped
                  fields, converting a {labelData.orderedItem} will
                  automatically transfer the information you have gathered to an{" "}
                  {labelData.invoicedItem}.
                </p>
              </div>
              {errorMessage.length ? (
                <div className="mb-6">
              ...
              {!layoutLoading && orderedItemListItems.length ? (
                <div className="bg-white p-6 w-full rounded-xl shadow-sm flex flex-row">
                  <OrderedItemConversionMappingList
                    quoteMappingExcludedFieldNames={
                      quoteMappingExcludedFieldNames
                    }
